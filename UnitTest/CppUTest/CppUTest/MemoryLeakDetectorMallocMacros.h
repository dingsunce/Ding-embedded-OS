 ...
 * code. It defines a preprocessor macro for malloc. This will pass additional information to the
...
/* This prevents the declaration from done twice and makes sure the file only #defines malloc, so it
 ...
 * Because of that, it can lead to portability issues by providing more than is available on the
 * local platform. For that reason, strdup is *not* implemented as a macro. If you still want to use
 * it, an easy implementation would be:
 *
